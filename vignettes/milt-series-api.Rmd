---
title: "MiltSeries API Reference"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{MiltSeries API Reference}
...
`MiltSeries` is the foundational data structure in milt. Every model, detector,
pipeline, and analysis function expects a `MiltSeries` as input and returns
results that reference one. This guide documents every way to create, inspect,
slice, transform, and combine `MiltSeries` objects.
...
| `metadata` | named list | `list()` | Arbitrary metadata attached to the series (e.g. `list(source = "sensor_A")`). Retrieved with `$metadata`. |
...
| `start` | date-like or NULL | `NULL` | Start of the window. Accepts `Date`, `POSIXct`, or a character string parseable by `lubridate::as_datetime()`. |
...
milt supports three kinds of covariates that can be attached to a series:

- **Static covariates** — scalar features that apply to the whole series (e.g. store size, country code).
- **Past covariates** — time-varying features known only up to the forecast origin (e.g. lagged external variables).
- **Future covariates** — time-varying features known over the entire forecast horizon (e.g. calendar events, planned promotions).
...
When `group_col` is set, most milt functions automatically apply per-group.
...
| `frequency` string | Observations per year | Typical `ts()` period |
|---|---|---|
| `"minutely"` | 525,600 | — |
| `"hourly"` | 8,760 | 8760 |
| `"daily"` | 365 | 365 |
...
You may also pass a bare positive integer (e.g. `frequency = 7` for daily data
with a weekly seasonal cycle).

---

## 9. Pipe-Friendly Design

All `milt_*` functions accept the series as their **first argument**, making
them fully compatible with the native pipe `|>`:
...
- `vignette("milt-eda-diagnostics")` — exploratory analysis of a series
- `vignette("milt-multi-series")` — multi-series and hierarchical workflows
- `vignette("milt-pipelines")` — feature engineering pipelines
