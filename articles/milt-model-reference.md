...
This page is the complete reference for every forecasting model
available in milt. Every model uses the same three-step interface:
...
Repeats the last full seasonal cycle. For monthly data this copies the
same month from the previous year. A strong baseline for series with
clear seasonality.
...
Extrapolates the average trend observed between the first and last
...
Searches over ARIMA(p,d,q)(P,D,Q) orders using AICc or BIC, with
optional seasonal differencing and stepwise search.
...
**When to use:** A solid default for univariate monthly or quarterly
series. Use `stepwise = FALSE` for a thorough search when fit time is
not a concern.
...
Fits an Error-Trend-Seasonality state-space model. The model string
`"ZZZ"` lets the algorithm choose the best combination automatically.
...
**When to use:** Robust for a wide range of univariate series. Works
well when the variance grows proportionally to the level (use `"ZZZ"` or
`"MAM"`).
...
Decomposes the series into two “theta lines” with different slopes and
combines their forecasts. Performs well on M3 competition data.
...
Wraps
[`forecast::stlf()`](https://pkg.robjhyndman.com/forecast/reference/forecast.stl.html).
Decomposes the series using STL (Seasonal and Trend decomposition using
Loess), forecasts the seasonally-adjusted series with ETS or ARIMA, then
re-seasonalises.
...
Handles multiple seasonalities, Box-Cox transformations, ARMA errors,
and damped trends. Computationally expensive but powerful for complex
seasonal patterns (e.g., hourly data with daily and weekly seasons).
...
Designed for **intermittent demand** series — where many observations
are zero with sporadic positive values. Separately smooths the non-zero
demand sizes and the intervals between them.
...
Designed for business time series with strong multi-seasonality and
irregular holidays.
...
All ML models convert the time series into a **supervised learning**
problem by building lag features from the training window. The target is
the next-step value; the features are lagged values at positions defined
by the `lags` argument.

### `xgboost` — XGBoost

Gradient-boosted decision trees. Fast, accurate, handles non-linearities
...
Microsoft’s gradient-boosting framework. Faster training and lower
memory use than XGBoost on large datasets, especially with many
features.
...
Linear model with L1 + L2 regularisation via `glmnet`. Performs
automatic lag selection when `lambda = NULL` (5-fold CV). Interpretable
and fast.
...
Non-parametric lag-based forecasting. Finds the `k` most similar
historical windows and averages their next steps.
...
All deep learning backends are built with [torch for
R](https://torch.mlverse.org/). They require `torch` to be installed
...
perceptron architecture with stacks of fully-connected blocks. Each
block produces a *backcast* (removing information from the input) and a
*forecast*; block outputs are summed. Achieves state-of-the-art results
on M4 competition without domain knowledge.
...
interpolation, reducing computation while improving long-horizon
accuracy.
...
Combines LSTMs and multi-head attention to capture both short-term
patterns (via LSTM encoder) and long-term dependencies (via
self-attention). Designed for multi-horizon probabilistic forecasting.
...
Dilated causal convolutions allow the receptive field to grow
exponentially with depth, capturing long-range dependencies without
recurrence.
...
Applies a Vision Transformer concept to time series: the input window is
split into non-overlapping **patches**, each patch is linearly embedded
into a token, learnable positional encodings are added, and a standard
Transformer encoder processes all patch tokens jointly. Excels at
...
Probabilistic LSTM-based model trained as an autoregressive likelihood
model. Returns Gaussian parameters (mean and standard deviation) at each
step, which are used to construct prediction intervals analytically.
